...
for file_num = 1:num_files
    ...
    fo = fitoptions('Method','NonLinearLeastSquares',...
                   'Lower',[0,0,0],...
                   'Upper',[inf,inf,inf],...
                   'StartPoint',[max(curves_row),1,max(inputs)/2]);
    ft = fittype('a/(1+exp(-b*(x-c)))-a/(1+exp(b*c))','options',fo);
    [curvefit,~] = fit(inputs,curves_row,ft);
    test_y = curvefit.a./(1+exp(-curvefit.b.*(rampInputs{1}-curvefit.c)))-curvefit.a./(1+exp(curvefit.b*curvefit.c));
